...
// CHECK: %q2 = qssa.apply<#gate.h> %q0
// CHECK-GENERIC: %q2 = "qssa.apply"(%q0) <{gate = #gate.h}> : (!qu.bit) -> !qu.bit
%q2 = qssa.apply<#gate.h> %q0

// CHECK: %q3 = qssa.apply<#gate.rz<0.5pi>> %q1
// CHECK-GENERIC: %q3 = "qssa.apply"(%q1) <{gate = #gate.rz<0.5pi>}> : (!qu.bit) -> !qu.bit
%q3 = qssa.apply<#gate.rz<0.5pi>> %q1

// CHECK: %q4, %q5 = qssa.apply<#gate.cx> %q2, %q3
// CHECK-GENERIC: %q4, %q5 = "qssa.apply"(%q2, %q3) <{gate = #gate.cx}> : (!qu.bit, !qu.bit) -> (!qu.bit, !qu.bit)
%q4, %q5 = qssa.apply<#gate.cx> %q2, %q3

%g1 = "test.op"() : () -> !instrument.type<1>

// CHECK: %q6 = qssa.dyn_apply<%g1> %q5
// CHECK-GENERIC: %q6 = "qssa.dyn_apply"(%g1, %q5) : (!instrument.type<1>, !qu.bit) -> !qu.bit
%q6 = qssa.dyn_apply<%g1> %q5
...
// CHECK-GENERIC: %{{.*}} = "qssa.circuit"() ({
// CHECK-GENERIC-NEXT: ^{{.+}}(%{{.*}} : !qu.bit):
// CHECK-GENERIC-NEXT:   %{{.*}} = "qssa.apply"(%{{.*}}) <{gate = #gate.x}> : (!qu.bit) -> !qu.bit
// CHECK-GENERIC-NEXT:   "qssa.return"(%{{.*}}) : (!qu.bit) -> ()
// CHECK-GENERIC-NEXT: }) : () -> !instrument.type<1>
%circuit2 = qssa.circuit() ({
^bb0(%arg0 : !qu.bit):
  %q8 = qssa.apply<#gate.x> %arg0
  qssa.return %q8
}) : () -> !instrument.type<1>
...
// CHECK-GENERIC-NEXT:   %{{.*}}, %{{.*}} = "qssa.apply"(%{{.*}}, %{{.*}}) <{gate = #gate.cx}> : (!qu.bit, !qu.bit) -> (!qu.bit, !qu.bit)
...
^bb0(%arg0 : !qu.bit, %arg1 : !qu.bit):
  %q9, %q10 = qssa.apply<#gate.cx> %arg0, %arg1
  qssa.return %q9, %q10
}) : () -> !instrument.type<2>
