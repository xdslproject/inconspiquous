...
// CHECK-NEXT:   %q_1 = qssa.apply<#gate.h> %q
// CHECK-NEXT:   %q_2 = gate.xz %z, %x
// CHECK-NEXT:   %q_3 = qssa.dyn_apply<%q_2> %q_1
// CHECK-NEXT:   func.return
// CHECK-NEXT: }

func.func @h_commute(%q : !qu.bit, %x : i1, %z : i1) {
  %g = gate.xz %x, %z
  %q_1 = qssa.dyn_apply<%g> %q
...
// CHECK-NEXT:   %q_1 = qssa.apply<#gate.id<1>> %q
...
func.func @id_commute(%q : !qu.bit, %x : i1, %z : i1) {
  %g = gate.xz %x, %z
  %q_1 = qssa.dyn_apply<%g> %q
  %q_2 = qssa.apply<#gate.id<1>> %q_1
  func.return
}

// CHECK:      func.func @x_commute(%q : !qu.bit, %x : i1, %z : i1) {
// CHECK-NEXT:   %g = gate.xz %x, %z
// CHECK-NEXT:   %q_1 = qssa.apply<#gate.x> %q
// CHECK-NEXT:   %q_2 = qssa.dyn_apply<%g> %q_1
// CHECK-NEXT:   func.return
// CHECK-NEXT: }
func.func @x_commute(%q : !qu.bit, %x : i1, %z : i1) {
  %g = gate.xz %x, %z
  %q_1 = qssa.dyn_apply<%g> %q
  %q_2 = qssa.apply<#gate.x> %q_1
  func.return
}

// CHECK:      func.func @y_commute(%q : !qu.bit, %x : i1, %z : i1) {
// CHECK-NEXT:   %g = gate.xz %x, %z
// CHECK-NEXT:   %q_1 = qssa.apply<#gate.y> %q
// CHECK-NEXT:   %q_2 = qssa.dyn_apply<%g> %q_1
// CHECK-NEXT:   func.return
// CHECK-NEXT: }
func.func @y_commute(%q : !qu.bit, %x : i1, %z : i1) {
  %g = gate.xz %x, %z
  %q_1 = qssa.dyn_apply<%g> %q
  %q_2 = qssa.apply<#gate.y> %q_1
  func.return
}

// CHECK:      func.func @z_commute(%q : !qu.bit, %x : i1, %z : i1) {
// CHECK-NEXT:   %g = gate.xz %x, %z
// CHECK-NEXT:   %q_1 = qssa.apply<#gate.z> %q
// CHECK-NEXT:   %q_2 = qssa.dyn_apply<%g> %q_1
// CHECK-NEXT:   func.return
// CHECK-NEXT: }
...
func.func @phase_dagger_commute(%q : !qu.bit, %x : i1, %z : i1) {
  %g = gate.xz %x, %z
  %q_1 = qssa.dyn_apply<%g> %q
  %q_2 = qssa.apply<#gate.s_dagger> %q_1
  func.return
}
...
// CHECK-NEXT:   %0 = arith.constant false
// CHECK-NEXT:   %1 = arith.constant false
// CHECK-NEXT:   %2, %3 = qssa.apply<#gate.cx> %q1, %q2
// CHECK-NEXT:   %4 = arith.xori %1, %x1 : i1
...
func.func @cx_commute(%q1 : !qu.bit, %q2 : !qu.bit, %x1 : i1, %z1 : i1, %x2 : i1, %z2 : i1) {
  ...
  %q1_1 = qssa.dyn_apply<%g1> %q1
  %q2_1 = qssa.dyn_apply<%g2> %q2
  %q1_2, %q2_2 = qssa.apply<#gate.cx> %q1_1, %q2_1
  func.return
}
