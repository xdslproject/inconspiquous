// RUN: quopt %s -p mbqc-legalize --verify-diagnostics --split-input-file | filecheck %s

%q0 = qu.alloc
%q1 = qu.alloc

%q0_1 = qssa.gate<#gate.x> %q0
// CHECK: A CZ gate can only follow allocations and CZ gates in a valid mbqc program, found qssa.gate
%q0_2, %q1_1 = qssa.gate<#gate.cz> %q0_1, %q1

// -----

%q0 = qu.alloc
%q1 = qu.alloc
// CHECK: Expected only CZ or Pauli gates, found #gate.cx
%q0_1, %q1_1 = qssa.gate<#gate.cx> %q0, %q1

// -----

%q0 = qu.alloc
// CHECK: Expected only XY measurements, found #measurement.comp_basis
%q0_1, %0 = qssa.measure %q0

// -----

%q0 = qu.alloc
%q0_1 = qssa.gate<#gate.x> %q0
// CHECK: A measurement can only follow allocations and CZ gates in a valid mbqc program, found qssa.gate
%q0_2, %0 = qssa.measure<#measurement.xy<0>> %q0_1

// -----
%q0 = qu.alloc
%q0_1 = qssa.gate<#gate.x> %q0
%a = angle.constant<0>
%m = measurement.dyn_xy<%a>
// CHECK: A measurement can only follow allocations and CZ gates in a valid mbqc program, found qssa.gate
%q0_2, %0 = qssa.dyn_measure<%m> %q0_1

// -----

// CHECK: Only expected dynamic Pauli gates, found #gate.cz
%g = instrument.constant #gate.cz

// -----

%q0 = qu.alloc
// CHECK: Unexpected operation qref.gate
qref.gate<#gate.x> %q0
